...
void* multiThread(void* thread_num)
{
    ...
    for( int px=0; px<width; ++px )
    {
        ...
        for( int py=*id; py<height; py+=nthreads )
        {
...
main( int argc, char **argv )
{
    ...
    pthread_t* thread_mem = (pthread_t*)malloc(sizeof(pthread_t)*nthreads);
    int* id = (int*)malloc(sizeof(int)*nthreads);
    int ret;
    for ( int i=0; i<nthreads; i++ )
    {
        id[i] = i;
        ret = pthread_create(&thread_mem[i], NULL, (void*)multiThread, (void*)&id[i]);
        if (ret)
        {
    ...
    for ( int px=0; px<width; ++px )
    {
        ...
        {
            printf( "%.0f %.0f %.0f\n",
                   scaled_color_entire[px][py][0], scaled_color_entire[px][py][1], scaled_color_entire[px][py][2] );
        }
    }
    printf("\n");
